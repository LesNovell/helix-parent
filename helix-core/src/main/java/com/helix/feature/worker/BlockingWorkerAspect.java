...
package com.helix.feature.worker;
 ...
 */
@Aspect
@DeclarePrecedence(value = "com.helix.feature.worker.BlockingWorkerAspect,*")
public class BlockingWorkerAspect {
     ...
     */
    @Before(value = "execution(public void com.helix.feature.worker.BlockingWorkerFeature.start(com.helix.core.server.Server)) "
            + "&& args(server)")
    ...
    @Around(value = "(execution(public * *(..)) && @annotation(blockingWorker)) || "
            + "(execution(public * *(..)) && within(@com.helix.feature.worker.BlockingWorker *) && @annotation(blockingWorker))")
